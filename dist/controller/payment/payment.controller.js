 ...
 */
export const confirmOrderPayment = async (orderIdOrReceipt, razorpayOrderId, paymentId, signature) => {
    const matchedOrders = [];
    ...
    if (matchedOrders.length === 0) {
        ...
        for (const o of createdOrders) {
            ...
            if (info && typeof info === "object") {
                if ((razorpayOrderId && (info.razorpay_order_id === razorpayOrderId || info.order_id === razorpayOrderId)) ||
 ...
 */
export const handleOrderPaymentFailure = async (orderIdOrReceipt, razorpayOrderId, paymentId, errorCode, errorDesc) => {
    const matchedOrders = [];
    // Strategy 1: Find by order ID(s) parsed from receipt/ID parameter
    if (orderIdOrReceipt) {
        const matches = orderIdOrReceipt.match(/\d+/g);
    ...
    if (matchedOrders.length === 0 && razorpayOrderId) {
        ...
        if (matchedOrders.length === 0) {
            ...
            for (const o of createdOrders) {
                const info = o.paymentInfo;
                if (info && typeof info === "object") {
                    if (info.razorpay_order_id === razorpayOrderId ||
...
export const verifyPayment = async (req, res) => {
    try {
        ...
        const updatedOrders = await confirmOrderPayment(receipt || order_id, order_id, payment_id, razorpay_signature);
    ...
    try {
        ...
        if (eventData.event === "order.paid" || eventData.event === "payment.captured") {
            ...
            if (razorpayOrderId && paymentId) {
                await confirmOrderPayment(receipt || razorpayOrderId, razorpayOrderId, paymentId, signature);
            }
        }
            ...
            if (razorpayOrderId && paymentId) {
                await handleOrderPaymentFailure(razorpayOrderId, razorpayOrderId, paymentId, errorCode, errorDesc);
            }
        }
