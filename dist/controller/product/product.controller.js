    ...
    try {
        ...
        if (typeof productData.categories === 'string') {
            try {
                ...
                productData.categories = productData.categories.split(',').map((id) => parseInt(id.trim())).filter((id) => !isNaN(id));
            }
        }
        ...
        if (productData.images && Array.isArray(productData.images)) {
            ...
            const base64Images = productData.images.filter((img) => typeof img === 'string' && (img.startsWith('data:image') || img.length > 100) // Base64 images are usually long strings
            );
            ...
            const existingUrls = productData.images.filter((img) => typeof img === 'string' && img.startsWith('http') && !img.startsWith('data:image'));
...
export const update = async (req, res) => {
    try {
        ...
        if (productData.images && Array.isArray(productData.images)) {
            ...
            const base64Images = productData.images.filter((img) => typeof img === 'string' && (img.startsWith('data:image') || (img.length > 100 && !img.startsWith('http'))));
            ...
            const existingUrls = productData.images.filter((img) => typeof img === 'string' && img.startsWith('http'));
        ...
        const imagesToDelete = existingImages.filter((oldImg) => !finalImageUrls.includes(oldImg));
        // Delete old images from S3 that are not in the new list
        if (imagesToDelete.length > 0) {
            try {
                await Promise.all(imagesToDelete.map((url) => deleteFromS3(url).catch((err) => {
