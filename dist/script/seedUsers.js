...
async function seedUsers() {
    ...
    try {
        ...
        for (const sellerData of sellers) {
            ...
            if (sellerCreated) {
                await sellerUser.addRole(sellerRole, { transaction: t });
                ...
                await jiffy.query(`INSERT INTO seller_profile (id, userId, businessName, gstNumber, address, city, state, zipCode, phone, createdAt, updatedAt) 
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`, {
                    replacements: [
                        verifiedSeller.id,
                        sellerUser.id,
                ...
                const sellerProfileRecord = await SellerProfile.findByPk(verifiedSeller.id, { transaction: t });
                ...
                console.log(`✅ Created complete seller profile for: ${sellerUser.phone_number}`);
            }
            else {
                console.log(`ℹ️  Seller user already exists: ${sellerUser.phone_number}`);
            }
        ...
        for (const buyerData of buyers) {
            ...
            if (buyerCreated) {
                await buyerUser.addRole(buyerRole, { transaction: t });
                ...
                console.log(`✅ Created buyer profile for: ${buyerUser.phone_number}`);
            }
            else {
