...
const validateAWSCredentials = () => {
    if (!AWS_ACCESS_KEY_ID || AWS_ACCESS_KEY_ID.trim() === '') {
        throw new Error("AWS_ACCESS_KEY_ID is not configured. Please set it in your .env file. See AWS_CREDENTIALS_SETUP.md for instructions.");
    }
    if (!AWS_SECRET_ACCESS_KEY || AWS_SECRET_ACCESS_KEY.trim() === '') {
        throw new Error("AWS_SECRET_ACCESS_KEY is not configured. Please set it in your .env file. See AWS_CREDENTIALS_SETUP.md for instructions.");
    }
    if (!BUCKET_NAME || BUCKET_NAME.trim() === '') {
        throw new Error("AWS_S3_BUCKET_NAME is not configured. Please set it in your .env file. See AWS_CREDENTIALS_SETUP.md for instructions.");
    }
};
 ...
 * @returns S3 URL of uploaded file
 */
export const uploadBase64ToS3 = async (base64String, folder = "products") => {
    try {
        const client = getS3Client();
 ...
 * @returns Array of S3 URLs
 */
export const uploadMultipleBase64ToS3 = async (base64Strings, folder = "products") => {
    try {
