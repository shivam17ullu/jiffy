...
export default class AdminService {
	...
	static async getSellerDetails(sellerId: number) {
	...
	static async getProducts(sellerId?: number, status?: "active" | "inactive") {
	...
	static async approveProduct(productId: number) {
	...
	static async deleteSeller(sellerProfileId: number) {
		...
		const user = (sellerProfile as any).User;
	...
	static async getPlatformRevenue() {
		...
		const sellerUserIds = sellers.map((s: any) => Number(s.userId));
		...
		const totalRevenue = parseFloat((overallResult[0] as any)?.totalRevenue) || 0;
		const totalOrders = parseInt((overallResult[0] as any)?.totalOrders) || 0;
		...
		const statsMap = new Map<number, { revenue: number; orderCount: number }>();
		for (const stat of orderStatsBySeller as any[]) {
		...
		const sellerBreakdown = sellers.map((profile: any) => {
	...
	static async getParticularSellerRevenue(sellerProfileId: number) {
	...
	static async getSellerDashboard(sellerProfileId: number) {
		...
		const totalRefundedAmount = parseFloat((refundResult[0] as any)?.totalRefund) || 0;
