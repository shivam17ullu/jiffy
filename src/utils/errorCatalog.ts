import { ApiError, FieldError } from "./ApiError.js";
import {
  ValidationError,
...
export const SELLER_ACCESS_ERRORS = {
  notActive:
    "Seller account is not active. Complete phone verification first.",
  pendingApproval: "Seller account is pending admin approval.",
} as const;

export const resolveError = (
  error: unknown
): { status: number; message: string; errors: FieldError[] } => {
  ...
  if (rawMessage.startsWith("Invalid status.")) {
    return {
      status: 400,
      message: "Invalid order status. Allowed values: created, confirmed, processing, shipped, delivered, cancelled, returned, refunded",
      errors: [
        {
          field: "status",
  ...
  if (
    rawMessage.includes("Order not found") ||
  ...
  if (catalog) {
    const errors: FieldError[] = catalog.field
